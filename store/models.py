...
class Category(models.Model):
    """
    Defines the data model for a category object.
    """
    class Meta:
...
class Product(models.Model):
    ...
    def clean(self):
        if self.rating is not None and (self.rating < 0 or self.rating > 5):
            raise ValidationError({'rating': 'Rating value must be \
                between 0 and 5'})
